...
var schedulelist = async function(){
    var result=[];
    
    await sql('SELECT distinct on (schedule_name) * FROM schedule AS a JOIN process AS b ON a.schedule_id = b.schedule_id')
        .then((data) => {            
            result = data.rows;  
        }, (error) => {
            result = null;
...
var scheduledetail = async function(schedule_id){
    ...
    await sql('select order_no,schedule.schedule_id,schedule.group_id,process.place_name from schedule join process on schedule.schedule_id= process.schedule_id  where schedule.schedule_id= $1', [schedule_id])
     .then((data) => {            
            result = data.rows;  
        }, (error) => {
            result = null;
...
//匯出
module.exports = {list,one,processlist,schedulelist,add,scheduledetail};
