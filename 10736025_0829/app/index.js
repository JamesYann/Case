           ...
           if(data==-9){
               ...
               event.reply('目前無計畫表，請先新建資料表!');
                ...
                for(i=0;i<10 && i<data.rows.length;i++){
                 ...
                 action.label='加入'+data.rows[i].schedule_name;
                 action.data=real_getdata+data.rows[i].schedule_id+'S';   //傳到POSTBACK讓事件以'S'識別要做選擇SCHEDULE加入景點
                 content.actions.push(action);

                 
                 content.text=data.rows[i].schedule_name;
